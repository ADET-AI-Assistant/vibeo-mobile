  ...
  return (
    <View style={styles.railContainer}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <MediaCard item={item} onPress={onNavigate} />
        )}
...
const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    if (isAnyError) {
      ...
      timeout = setTimeout(() => {
        setToastMessage(
          "Slow internet connection or server unreachable. Please wait...",
        );
      }, 6000);
    } else {
  ...
  const heroItems = useMemo(
    () =>
      ((trending.data?.pages[0]?.results || []) as HeroMedia[])
        .filter(
          (item) =>
            (getMediaType(item) === "movie" || getMediaType(item) === "tv") &&
            item.backdrop_path,
        )
        .slice(0, 6),
    [trending.data],
    ...
    const loadTrailers = async () => {
      ...
      const trailerEntries = await Promise.all(
        heroItems.map(async (item) => {
          try {
            const payload = await tmdbApi.getVideos(
              getMediaType(item),
              item.id,
            );
            const trailer =
  ...
  const moveHero = (direction: -1 | 1) => {
    ...
    const nextIndex = Math.max(
      0,
      Math.min(heroItems.length - 1, activeHeroIndex + direction),
    );
  ...
  const openTrailer = async (item: HeroMedia) => {
    ...
    if (!trailerKey) {
      Alert.alert(
        "Trailer unavailable",
        "No trailer found for this title yet.",
      );
      return;
    }
              ...
              <View style={styles.dotRow}>
                {heroItems.map((item, index) => (
                  <View
                    ...
                    style={[
                      styles.dot,
                      index === activeHeroIndex ? styles.dotActive : undefined,
                    ]}
                  />
                ))}
              </View>
